...
from Functions.Fed_algorithm import FedAvg, trimmed_and_thresholded_aggregation, subtract_weights, add_weights, \
    remove_anomalies, krum_aggregation, remove_anomalies_kmeans
...
from SplitFed_Security.Functions.class_flipping_df_method import replace_label1_with_label2_on_df, \
    random_select_poisoning_users, poison_data, cifar10_to_dataframe, poison_data_random, poison_data_model, \
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    if count1 == len_batch:
        ...
        if len(idx_collect) == num_users * frac:
            ...
            # 异常检测
            # 使用异常检测移除异常客户端
            # anomaly_threshold = 1  # 自定义阈值
            # w_locals_server = remove_anomalies(w_locals_server, w_glob_server, anomaly_threshold)
            # 使用基于统计的方法
            # w_locals_server = remove_anomalies(w_locals_server, w_glob_server, threshold_method='statistical',
            #                                    std_multiplier=1)

            # # 或使用基于聚类的方法
            w_locals_server,smallest_cluster_indices = remove_anomalies_kmeans(w_locals_server, w_glob_server, n_clusters=2, remove_ratio=0.2)


            # ================== 使用不同的聚合算法 =================
            print("w_local_server的长度",len(w_locals_server))
...
if dataset_choice == 'HAM10000':
    print(dataset_train.df.shape[0])
...
for iter in range(epochs):
    ...
    for idx in idxs_users:
        is_attacker = idx in attackers_indices if iter == 0 else None
        print("is_attaker",is_attacker)
    ...
    # # 或使用基于聚类的方法
    w_locals_client, smallest_cluster_indices = remove_anomalies_kmeans(w_locals_client, w_glob_client, n_clusters=2,
                                                                        remove_ratio=0.3)
...
file_name = program + "_label_flipping_random_" + str(poisoned_frac) + "_" + dataset_choice + ".xlsx"
