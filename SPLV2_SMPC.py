...
attackers_frac = 0.1  # Modify this value according to your requirements
...
# 在训练循环开始前，创建虚拟工作机以模拟客户端之间的计算过程
hook = sy.TorchHook(torch)
clients = [sy.VirtualWorker(hook, id=f"client_{i}") for i in range(num_users)]
# 将客户端的虚拟工作机设置为已知工作机
for i in range(num_users):
    for j in range(num_users):
        if i != j:
            clients[i].add_worker(clients[j])
...
for iter in range(epochs):
    ...
    for idx in idxs_users:
        ...
        # 对本地模型参数进行加密
        w_client_enc = {}
        for key, param in w_client.items():
            w_client_enc[key] = param.fix_precision().share(clients[idx], *clients[:idx], *clients[idx + 1:],
                                                            crypto_provider=clients[-1])
